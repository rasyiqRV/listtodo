...
import 'package:url_launcher/url_launcher.dart'; // Import Launcher
import 'main.dart';
...
// ==========================================
// 2. WIDGET ITEM (DENGAN FITUR INOVATIF)
// ==========================================
class TaskItemWidget extends StatelessWidget {
  final Task task;
  const TaskItemWidget({super.key, required this.task});

  // --- LOGIC 1: FITUR SHARE (WHATSAPP/EMAIL) ---
  void _shareTask(BuildContext context) {
    final String text =
        "Halo! Saya ingin berbagi tugas:\n\n"
        "📌 *${task.title}*\n"
        "📅 Tanggal: ${task.date}\n"
        "📂 Kategori: ${task.category}\n"
        "📝 Catatan: ${task.description}\n\n"
        "Dikirim via *Your Memo App*";
    Share.share(text);
  }

  // --- LOGIC 2: DETEKSI NOMOR TELEPON (REDIRECT CALL) ---
  String? _extractPhoneNumber(String description) {
    // Regex mencari angka 08... minimal 10 digit
    RegExp regExp = RegExp(r'\b08[0-9]{8,11}\b');
  ...
  Widget build(BuildContext context) {
    ...
    String? phoneNumber = _extractPhoneNumber(task.description);

    return Padding(
      padding: const EdgeInsets.only(bottom: 16),
      child: Slidable(
        key: ValueKey(task.id),
        // --- Geser Kanan (Share & Edit) ---
        startActionPane: ActionPane(
          motion: const ScrollMotion(),
          children: [
            SlidableAction(
              onPressed: (context) => _shareTask(context),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              "Additional Notes (Add phone number to enable call)",
              style: GoogleFonts.poppins(
                fontSize: 14,
                fontWeight: FontWeight.w600,
                color: Colors.grey[600],
            ...
            _buildInputContainer(
              child: TextField(
                ...
                style: GoogleFonts.poppins(),
                decoration: InputDecoration(
                  hintText: "E.g. Call Budi at 08123456789",
                  hintStyle: GoogleFonts.poppins(color: Colors.grey[400]),
                  border: InputBorder.none,
                ),
              ),
